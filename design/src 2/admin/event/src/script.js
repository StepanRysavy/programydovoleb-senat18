...
export default {
  ...
  methods: {
    ...
    combine: function () {
      ...
      if (this.data) {
        ...
        if (this.data.opened) {
          ...
          this.full.opened = {special: [], note: {en: "", cs: ""}};
        }
        ...
        if (this.data.updates) this.full.updates = this.data.updates || [];
        ...
        this.full.meta = {type: 'event', category: [], tags: [], block: 1, house: 1, priority: 3};
        this.full.opened = {special: [], note: {cs: "", en: ""}};
    ...
    handleClick_UpdateRemove: function (index) {
      this.full.events.splice(index, 1);
    },
    handleClick_UpdateAdd: function () {

      var obj = {
        content: {
          cs: this.$refs.update_content_cs.value,
          en: this.$refs.update_content_en.value,
        },
        name: {
          cs: this.$refs.update_name_cs.value,
          en: this.$refs.update_name_en.value,
        },
        date: new Date(this.$refs.update_date.value).getTime(),
        source: {
          src: this.$refs.update_source_src.value,
          type: this.$refs.update_source_type.value,
        }
      }

      this.full.updates.push(obj);
    },
